...
| Data Engineering | Approved catalog tags file | Runs data pipelines, applies tags, verifies them, and publishes governed objects |
...
7. Data Engineering runs the data-producing pipeline in protected schemas.
8. The tag applier reconciles the approved tags file and verifies assignments.
9. ABAC policy tests execute as authorized and unauthorized test principals.
10. Publication occurs only after all required verification succeeds.

Tag definitions must exist before policies reference them. Policies must exist
before tagged objects are published to consumers.
