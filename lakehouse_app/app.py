                ...
                if master in r:
                    ...
                    for line in lines:
                        ...
                        if (('?' in line or ':' in line or 
                            ...
                            'choose' in line.lower() or
                            'continue' in line.lower() or
                            'press' in line.lower()) and 
...
def handle_onboard_form():
    ...
    json_data = {
        ...
        "dlt_meta_schema": request.form.get('dlt_meta_schema', 'dlt_meta_dataflowspecs_4e6c360d3e5c4b5ca6687fec8ffe2e14'),
    ...
    f"python {current_directory}src/cli.py '{json_string}'",
    shell=True,
    capture_output=True,
    text=True
    )
...
def run_demo():
    ...
    result = subprocess.run(
    f"python {current_directory}/{demo_file} --uc_catalog_name {uc_name} --profile DEFAULT",
    shell=True,
    capture_output=True,
    text=True
    )
