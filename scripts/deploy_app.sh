#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
# scripts/deploy_app.sh — deploy the dlt-meta lakehouse_app to Databricks Apps
#
# Why this script exists
# ──────────────────────
# The `lakehouse_app/app.py` Flask routes (/onboarding, /deploy, /rundemo)
...
# repo at the deployment source-code-path (Mode A in lakehouse_app/start.sh).
#
...
#   - Local edits to demo/, src/, lakehouse_app/ are reflected on every run.
...
[[ -f lakehouse_app/start.sh ]]        || { echo "lakehouse_app/start.sh not found" >&2; exit 1; }
[[ -f lakehouse_app/requirements.txt ]] || { echo "lakehouse_app/requirements.txt not found" >&2; exit 1; }
[[ -f lakehouse_app/app.py ]]          || { echo "lakehouse_app/app.py not found" >&2; exit 1; }
...
# code-path root; lakehouse_app/start.sh detects Mode A (full repo) and runs
# the Flask app from the repo root so demo/ and src/ resolve correctly.
command: [
  "bash",
  "lakehouse_app/start.sh"
]
EOF

cp "$REPO_ROOT/lakehouse_app/requirements.txt" "$STAGING/requirements.txt"
...
# mounts it as a regular FILE, and ``lakehouse_app/start.sh`` restores the
