...
class WorkspaceInstaller:
    ...
    def _configure(self):
        ...
        self._config = WorkspaceConfig(
            ...
            cloud_provider_name=self._choice_from_dict("Provide cloud provider name", ['aws', 'azure', 'gcp']),
            dlt_meta_operation=self._choice_from_dict("Provide dlt meta operation", ['onboard', 'deploy']),
        )
        if self._config.dlt_meta_operation == 'onboard':
            self._load_onboard_config()
        if self._config.dlt_meta_operation == "deploy":
